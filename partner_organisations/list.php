...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $sql = "INSERT INTO partner_organisations 
                    (name, organisation_type, contact_person, email, phone, address, website, notes) 
                    VALUES (:name, :type, :contact, :email, :phone, :address, :website, :notes)";

            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                    ':name' => $form_data['name'],
                    ':type' => $form_data['organisation_type'],
                    ':contact' => $form_data['contact_person'] ?: null,
                    ':email' => $form_data['email'] ?: null,
                    ':phone' => $form_data['phone'] ?: null,
                    ':address' => $form_data['address'] ?: null,
                    ':website' => $form_data['website'] ?: null,
                    ':notes' => $form_data['notes'] ?: null
            ]);

            $_SESSION['success_message'] = "Organisation '{$form_data['name']}' has been added successfully!";
            header('Location: list.php');
            exit;

        } catch (PDOException $e) {
            $errors['database'] = 'Failed to add organisation. Please try again.';
...
<head>
    ...
    <title>Add Partner Organisation - SafePaws</title>
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h3><i class="bi bi-building-add"></i> Add New Partner Organisation</h3>
                </div>
                <div class="card-body">
                    ...
                    <form method="POST" action="" novalidate>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select <?= isset($errors['organisation_type']) ? 'is-invalid' : '' ?>"
                                ...
                                foreach ($types as $type):
                                    ?>
                                    <option value="<?= $type ?>" <?= (isset($form_data['organisation_type']) && $form_data['organisation_type'] == $type) ? 'selected' : '' ?>>
                                        <?= $type ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="contact_person" name="contact_person"
                                   value="<?= htmlspecialchars($form_data['contact_person'] ?? '') ?>" maxlength="100">
                        </div>

                        <div class="mb-3">
                            <label for="email" class="form-label">Email</label>
                            <input type="email" class="form-control <?= isset($errors['email']) ? 'is-invalid' : '' ?>"
                                   id="email" name="email" value="<?= htmlspecialchars($form_data['email'] ?? '') ?>" maxlength="100">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="tel" class="form-control" id="phone" name="phone"
                                   value="<?= htmlspecialchars($form_data['phone'] ?? '') ?>" maxlength="20">
                        </div>

                        <div class="mb-3">
                            <label for="address" class="form-label">Address</label>
                            <input type="text" class="form-control" id="address" name="address"
                                   value="<?= htmlspecialchars($form_data['address'] ?? '') ?>" maxlength="255">
                        </div>
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-plus-circle"></i> Add Organisation
                            </button>
                        </div>
                    </form>
                </div>
